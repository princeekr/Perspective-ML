      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-16">
          {authors.map((author, index) => (
            <div key={author.name} className={`rounded-2xl bg-card p-5 sm:p-8 hover:shadow-xl transition-all duration-300 animate-slide-up stagger-${Math.min(index + 2, 6)}`}>
              <div className="flex items-start gap-4 sm:gap-6 mb-4 sm:mb-6">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-xl sm:text-2xl font-bold mb-1">{author.name}</h3>
                  <p className="text-accent font-medium mb-2 sm:mb-3 text-sm sm:text-base">{author.role}</p>
                  <p className="text-xs sm:text-sm text-muted-foreground">{author.articles} articles published</p>
                </div>
              </div>
